...
1. Рекомендуйте товары/услуги и КОНКРЕТНЫХ специалистов из списка ниже.
2. Для вставки карточки товара используйте СТРОГО формат: SERVICE_CARD[id]
3. Для вставки профиля специалиста используйте СТРОГО формат: PROVIDER_CARD[username]
4. ВАЖНО: Пишите карточку с новой строки. НЕ ставьте точки, скобки, запятые или другие знаки препинания сразу после закрывающей скобки ]. 
5. Пример плохого ответа: "Вот этот товар: SERVICE_CARD[123]."
6. Пример хорошего ответа: "Рекомендую этого специалиста:\nPROVIDER_CARD[ivan_design]\nОн отлично справится."

ДОСТУПНЫЕ УСЛУГИ:
${input.services.slice(0, 20).map(s => `- ID: ${s.id}, ${s.title}, ${s.price} TJS, Категория: ${s.category}`).join('\n')}

ДОСТУПНЫЕ СПЕЦИАЛИСТЫ:
${input.providers.map(p => `- Username: ${p.username}, Имя: ${p.name}, Био: ${p.bio}`).join('\n')}
`
        }
      ];
      ...
      if (input.photoDataUri) {
        messages.push({
          role: 'user',
          content: [
            { type: 'text', text: input.message || 'Что на этом фото? Подбери подходящие услуги или специалистов Findora.' },
            { 
              type: 'image_url', 
              image_url: { 
